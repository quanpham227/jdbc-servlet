                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                            <c:forEach var="item" items="${model.listResult}">
                                                <tr>
                                                    <td><input type="checkbox" id="checkAll_${item.id}" value="${item.id}"></td>
                                                    <td>${item.title}</td>
...
<script type="text/javascript">
    ...
    window.pagObj = $('#pagination').twbsPagination({
      ...
      onPageClick: function (event, page) {
        if (currentPage != page) {
          ...
          $('#type').val('list');
          $('#page').val(page);
          $('#formSubmit').submit(); // để gọi 1 đối tương trong jqery ta gọi id
        }
      }
    });
  });
  ...
  $("#btnDelete").click(function(){
    ...
    var ids = $('tbody input[type=checkbox]:checked').map(function () {
      return $(this).val();
    }).get(); // nó sẽ tìm kiếm trong form kiểm tra từng thẻ , chỗ nào có input type là checkbox(đượcc tích chọn) sẽ chuyển vào 1 mảng

    // muốn đẩy lên server phải xử lý như sau:
    // ở AbstracModel có 1 biến là ids gán ids = biến ids
    data['ids'] = ids;
    deleteNew(data);
